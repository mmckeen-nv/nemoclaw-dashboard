          ...
          {!isDestroyMode && selectedSandbox && (
            <>
              ...
              <ConfigurationPanel
                ...
                onSave={async (config) => {
                  // TODO: Call API to apply configuration to OpenShell
                  console.log('Saving configuration for', selectedSandbox, config)
                  // For now, just show it works
                  alert(`Configuration saved for ${selectedSandbox}!\n\nThis will call the OpenShell API to apply:\n- Disk Access: ${config.diskAccess}\n- Network: ${config.networkAccess}\n- GPU: ${config.gpuEnabled}\n- Memory: ${config.memoryLimit}\n- CPU: ${config.cpuLimit}`)
                }}
              />
            </>
          )}
